...
<div class="max-w-5xl mx-auto bg-white rounded-2xl shadow p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
    ...
    >
        ...
        @if(count($images))
            <img :src="'{{ asset('storage') }}/' + {{ json_encode($images) }}[active]" alt="{{ $product->name }}"
                 class="rounded-2xl object-cover w-full h-64 sm:h-80 md:h-[400px]">

            @if(count($images) > 1)
                <button @click="active = active === 0 ? {{ count($images) - 1 }} : active - 1"
                        class="absolute left-3 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full p-2 shadow">‹</button>
                <button @click="active = active === {{ count($images) - 1 }} ? 0 : active + 1"
                        class="absolute right-3 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full p-2 shadow">›</button>
            @endif
            ...
            @if(count($images) > 1)
                <div class="flex gap-2 mt-3 justify-center items-center overflow-x-auto">
                    <template x-for="(image, index) in visibleImages" :key="index">
                        <img :src="'{{ asset('storage') }}/' + image" @click="active = startIndex + index"
    ...
    <div>
        ...
        <p class="text-xl sm:text-2xl font-semibold mb-4">Dkk {{ number_format($product->price, 2) }}</p>
        <p class="text-gray-700 mb-4 sm:mb-6">{{ $product->description }}</p>
